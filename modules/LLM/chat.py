...
from .fetch_LLM import LLM

# /llm command stuffs
async def llm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    print(datetime.datetime.now(), "\t", "[chat] AI chat called.")
...
async def AI_chat(update: Update, context: ContextTypes.DEFAULT_TYPE, message_id: str):
    print(datetime.datetime.now(), "\t", "[chat] A new thread started to handle AI chat.")
    ...
    if update.message.reply_to_message:
        ...
        print(datetime.datetime.now(), "\t", "[chat] Showing usage")
        ...
        return
    print(datetime.datetime.now(), "\t", "[chat] Asking: ", usr_prompt)
    # pre_filters here
    ...
    for keyword in pre_filter:
        if keyword in usr_prompt.lower():
            ...
            print(datetime.datetime.now(), "\t", "[chat] Filter triggered: " + keyword + " in usr_prompt")
            return
    if usr_prompt is None:
        print(datetime.datetime.now(), "\t", "[chat] User prompt is None, showing usage.")
    ...
    for keyword in pre_filter:
        if keyword in AI_result.lower():
            await context.bot.edit_message_text(chat_id=update.effective_chat.id, text="Generated by AI, may not be accurate.\n咱不懂喵...", message_id=message_id)
            print(datetime.datetime.now(), "\t", "[chat] Filter triggered: " + keyword + " in AI_result")
            return
    ...
    print(datetime.datetime.now(), "\t", "[chat] Response sent.")
